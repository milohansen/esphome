...
from esphome.const import CONF_PLATFORM, CONF_ID, CONF_PIN, CONF_ADDRESS, CONF_ESPHOME, CONF_NAME
from esphome.helpers import write_file_if_changed, indent
...
def generate_bridge_cpp(legacy_info, native_info):
    ...
    for comp_id, info in native_info.items():
        if info['platform'] == 'gpio_switch':
            ...
            code.append('    rust_set_@comp_id@_state(state);'.replace('@comp_id@', comp_id))
            code.append('  }')
            ...
            code.append(f'class {comp_id}Proxy : public esphome::binary_sensor::BinarySensor {{')
            code.append(' public:')
            code.append('  std::atomic<bool> shadow_state{false};')
            code.append('};')
            code.append(f'{comp_id}Proxy* {comp_id}_proxy = new {comp_id}Proxy();')
    ...
    code.append('    auto* component = static_cast<esphome::Component*>(component_ptr);')
    ...
    for comp_id, info in native_info.items():
        ...
        code.append(f'    {comp_id}_proxy->shadow_state.store(state, std::memory_order_relaxed);')
...
def generate_bridge_rs(legacy_info, native_info):
    ...
    for comp_id in legacy_info:
        code.append(f'    pub fn create_{comp_id}() -> *mut c_void;')
    code.append('}')
    code.append('')
    ...
    for comp_id in native_info:
        code.append(f'extern "C" {{ pub fn update_shadow_{comp_id}(state: bool); }}')

    # FFI for C++ calling Rust
    for comp_id, info in native_info.items():
        if info['platform'] == 'gpio_switch':
            code.append(f'pub static {comp_id.upper()}_CMD: Channel<CriticalSectionRawMutex, crate::components::gpio_switch::SwitchCommand, 8> = Channel::new();')
            code.append('#[no_mangle]')
            code.append(f'pub extern "C" fn rust_set_{comp_id}_state(state: bool) {{')
            code.append(f'    let cmd = if state {{ crate::components::gpio_switch::SwitchCommand::TurnOn }} else {{ crate::components::gpio_switch::SwitchCommand::TurnOff }};')
...
[dependencies]
embassy-executor = {{ version = "0.7.0", features = ["executor-thread", "task-arena-size-12288"] }}
...
def generate_main_rs(config, legacy_info, native_info):
    ...
    if has_i2c(config):
        ...
        for i, i2c_conf in enumerate(i2c_configs):
            ...
            code.append(f'        &clocks')
            code.append(f'    );')
            if i == 0:
                code.append(f'    let async_i2c = bridge::AsyncI2cBus::new(i2c{i});')
                code.append(f'    bridge::I2C_BUS.set(async_i2c).unwrap();')
            code.append('')
    ...
    for comp_id in legacy_info:
        ...
        code.append(f'        let {comp_id}_wrapper = LegacyWrapper::new({comp_id}_ptr, "{comp_id}");')
        code.append(f'        spawner.spawn(run_legacy_component({comp_id}_wrapper)).unwrap();')
        code.append(f'    }}')
        legacy_ptrs.append(f'("{comp_id}", {comp_id}_ptr)')

    if legacy_ptrs:
        code.append(f'    unsafe {{ bridge::LEGACY_COMPONENTS = &[{", ".join(legacy_ptrs)}]; }}')

    # Instantiate native components
    for comp_id, info in native_info.items():
        if info['platform'] == 'gpio_switch':
            pin = info['conf'].get('pin', 0)
            code.append(f'    let mut {comp_id} = crate::components::gpio_switch::GpioSwitch::new(')
            code.append(f'        "{comp_id}", io.pins.gpio{pin}.into(), esp_hal::gpio::Level::Low,')
            code.append(f'        &bridge::{comp_id.upper()}_CMD, bridge::update_shadow_{comp_id}')
            code.append(f'    );')
            code.append(f'    spawner.spawn(run_{comp_id}({comp_id})).unwrap();')
        elif info['platform'] == 'gpio_binary_sensor':
            pin = info['conf'].get('pin', 0)
            code.append(f'    let mut {comp_id} = crate::components::gpio_binary_sensor::GpioBinarySensor::new(')
            code.append(f'        "{comp_id}", io.pins.gpio{pin}.into(), esp_hal::gpio::Pull::Up,')
    ...
    for comp_id, info in native_info.items():
        if info['platform'] == 'gpio_switch':
            code.append(f'#[embassy_executor::task]')
            code.append(f'async fn run_{comp_id}(mut component: crate::components::gpio_switch::GpioSwitch) {{')
            code.append(f'    component.run().await;')
            code.append('}')
        elif info['platform'] == 'gpio_binary_sensor':
            code.append(f'#[embassy_executor::task]')
            code.append(f'async fn run_{comp_id}(mut component: crate::components::gpio_binary_sensor::GpioBinarySensor) {{')
            code.append(f'    component.run().await;')
            code.append('}')
    code.append('')
...
async def generate(config):
    ...
    for name, component, conf in legacy:
        ...
        if comp_id:
            setup_code, global_code = await capture_component_code(name, component, conf)
    ...
    # bridge.cpp
    write_file_if_changed(build_dir / "bridge.cpp", generate_bridge_cpp(legacy_info, native_info))
    # bridge.h (needed for bindgen)
    write_file_if_changed(build_dir / "bridge.h", '#include "esphome.h"\nextern "C" {\nvoid call_component_loop(void* ptr);\nvoid call_component_setup(void* ptr);\n}\n')
    # bridge.rs
    write_file_if_changed(build_dir / "bridge.rs", generate_bridge_rs(legacy_info, native_info))
    # main.rs
    write_file_if_changed(build_dir / "main.rs", generate_main_rs(config, legacy_info, native_info))
    # Cargo.toml
